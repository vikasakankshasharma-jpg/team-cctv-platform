...
async function check() {
  ...
  const c8 = data8.plans.premium.items.filter(i => i.display_name.toLowerCase().includes("camera"));
  console.log("Premium Cameras for 8MP request:", c8.map(c => c.display_name));
  ...
  const c5 = data5.plans.premium.items.filter(i => i.display_name.toLowerCase().includes("camera"));
  console.log("Premium Cameras for 5MP request:", c5.map(c => c.display_name));
}
check();
