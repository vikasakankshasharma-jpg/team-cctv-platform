...
export function ActivePipeline({ leads }: { leads: Lead[] }) {
  ...
  return (
    <div className="grid grid-cols-1 gap-4">
      {leads.map((lead) => (
        <div key={lead.id} className="bg-white/5 backdrop-blur-xl border border-white/10 p-6 rounded-[28px] hover:bg-white/10 transition-all group flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-start gap-4 flex-1 min-w-0">
            <div className="w-12 h-12 rounded-[18px] bg-white/5 flex items-center justify-center text-zinc-400 group-hover:bg-blue-500/10 group-hover:text-blue-500 transition-colors shrink-0 text-lg font-black border border-white/5">
          ...
          <div className="flex items-center gap-2 shrink-0 flex-wrap sm:flex-nowrap">
            ...
            <Link href={`/salesperson/leads`} className="w-10 h-10 rounded-2xl bg-white/10 border border-white/10 flex items-center justify-center text-white hover:bg-blue-600 transition-all shadow-lg hover:border-blue-500/50">
