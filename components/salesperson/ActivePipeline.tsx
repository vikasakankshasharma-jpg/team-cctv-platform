  ...
  return (
    <div className="grid grid-cols-1 gap-4">
      {leads.map((lead) => {
        ...
        return (
          <div key={lead.id} className={`backdrop-blur-xl border p-6 rounded-[28px] transition-all group flex flex-col sm:flex-row sm:items-center justify-between gap-4 ${isBroadcast ? 'bg-amber-500/10 border-amber-500/30' : 'bg-white/5 border-white/10 hover:bg-white/10'}`}>
            <div className="flex items-start gap-4 flex-1 min-w-0">
              <div className={`w-12 h-12 rounded-[18px] flex items-center justify-center transition-colors shrink-0 text-lg font-black border ${isBroadcast ? 'bg-amber-500/20 text-amber-500 border-amber-500/30' : 'bg-white/5 text-zinc-400 group-hover:bg-blue-500/10 group-hover:text-blue-500 border-white/5'}`}>
            ...
            <div className="flex items-center gap-2 shrink-0 flex-wrap sm:flex-nowrap">
              {isBroadcast ? (
                <button 
                  ...
                  className="flex items-center gap-2 px-6 py-3 rounded-2xl bg-amber-500 text-black font-black text-[10px] uppercase tracking-widest hover:bg-amber-400 shadow-[0_0_20px_rgba(245,158,11,0.4)] transition-all hover:scale-105"
                >
