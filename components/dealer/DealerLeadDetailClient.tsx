      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
          <div>
            ...
            <div className="flex items-center gap-3">
               <h1 className="text-3xl font-black text-zinc-900 dark:text-white">{lead.customer_name}</h1>
             ...
             <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-4 shadow-sm">
                ...
                <div className="flex items-center gap-2">
                  <select 
                    ...
                    className="flex-1 bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-lg py-2 px-3 text-xs font-bold text-zinc-900 dark:text-white outline-none focus:ring-2 focus:ring-blue-500/20 transition-all"
                  >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-6 shadow-sm">
                 ...
                 <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div className="space-y-1">
                       ...
                       <a href={`tel:+91${lead.mobile_number}`} className="text-lg font-black text-zinc-900 dark:text-white hover:text-blue-600 transition-colors flex items-center gap-2">
              ...
              <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-6 shadow-sm">
                 ...
                 <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                    <div className="p-4 bg-zinc-50 dark:bg-zinc-950 rounded-xl border border-zinc-100 dark:border-zinc-800">
                       ...
                       <p className="text-sm font-black text-zinc-900 dark:text-white capitalize">{lead.property_type}</p>
                    </div>
                    <div className="p-4 bg-zinc-50 dark:bg-zinc-950 rounded-xl border border-zinc-100 dark:border-zinc-800">
                       <p className="text-[9px] font-black text-zinc-400 uppercase tracking-widest mb-1">Technology</p>
                       <p className="text-sm font-black text-zinc-900 dark:text-white">{lead.technology_choice}</p>
                    </div>
                    <div className="p-4 bg-zinc-50 dark:bg-zinc-950 rounded-xl border border-zinc-100 dark:border-zinc-800">
                       <p className="text-[9px] font-black text-zinc-400 uppercase tracking-widest mb-1">Cabling Status</p>
                       <p className="text-sm font-black text-zinc-900 dark:text-white">{lead.cabling_done ? "Done" : "Pending"}</p>
                    </div>
                    <div className="p-4 bg-zinc-50 dark:bg-zinc-950 rounded-xl border border-zinc-100 dark:border-zinc-800">
                       <p className="text-[9px] font-black text-zinc-400 uppercase tracking-widest mb-1">Site Visit</p>
                       <p className="text-sm font-black text-zinc-900 dark:text-white">{lead.site_visit_date ? "Requested" : "No"}</p>
                    </div>
                 </div>
              </div>
              ...
              <div className="space-y-4">
                 ...
                 {quotes.length === 0 ? (
                    ...
                    <div className="space-y-3">
                       {quotes.map((q) => (
                          <div key={q.id} className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-5 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4 group hover:border-indigo-500/50 transition-all">
                             <div className="flex items-center gap-4">
                                ...
                                <div>
                                   <p className="text-sm font-black text-zinc-900 dark:text-white">Quote #{q.id.slice(-6).toUpperCase()}</p>
                                   <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest">{formatDate(q.created_at)}</p>
                                </div>
                             </div>
                             <div className="flex items-center gap-6">
                                <div className="text-right">
                                   <p className="text-[10px] font-black text-zinc-400 uppercase tracking-widest mb-0.5">Total Quote</p>
                                   <p className="text-base font-black text-zinc-900 dark:text-white">₹{fmt(q.total_payable || 0)}</p>
                                </div>
                                <a 
                                  href={`/quote/${lead.id}/review/${q.id}`} 
                                  target="_blank" 
                                  className="p-2 bg-zinc-50 dark:bg-zinc-950 border border-zinc-100 dark:border-zinc-800 rounded-lg text-zinc-400 hover:text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 transition-all"
           ...
           <div className="space-y-6">
              <div className="bg-emerald-50 dark:bg-emerald-900/10 border border-emerald-200 dark:border-emerald-800/40 rounded-2xl p-6">
              ...
              <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-6 shadow-sm">
                 ...
                 <ul className="space-y-4">
                    {[
                       { icon: Phone, text: "Call the customer within 2 hours of receiving the lead.", color: "text-blue-500" },
                       { icon: Calendar, text: "Coordinate a physical site visit for exact wiring measurement.", color: "text-purple-500" },
                       { icon: CheckCircle2, text: "Update lead status to 'Won' once payment is collected.", color: "text-emerald-500" },
                    ].map((item, i) => (
                       <li key={i} className="flex gap-3">
                          <item.icon className={`w-4 h-4 shrink-0 mt-0.5 ${item.color}`} />
                          <p className="text-[11px] text-zinc-600 dark:text-zinc-400 font-semibold leading-normal">{item.text}</p>
                       </li>
                    ))}
                 </ul>
              </div>
