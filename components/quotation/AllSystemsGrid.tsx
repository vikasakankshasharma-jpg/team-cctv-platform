  ...
  const allKits = useMemo(() => {
    ...
    if (selection.resolution_preference && selection.resolution_preference !== "all") {
      cameras = cameras.filter(p => {
        const res = String(p.resolution_mp).toUpperCase().trim();
        return res === selection.resolution_preference;
      });
    }
    ...
    if (selection.requested_features && selection.requested_features.length > 0) {
      cameras = cameras.filter(cam => {
        ...
        const check = (tag: string) => {
          if (tag === "mic") {
            return camFeats.some(f => f.includes("mic") || f.includes("audio")) || name.includes("mic") || name.includes("audio");
          }
          if (tag === "color") {
            return camFeats.some(f => f.includes("color")) || name.includes("color");
          }
          if (tag === "ptz") {
            return camFeats.some(f => f.includes("ptz")) || name.includes("ptz");
          }
              ...
              {/* Content Section */}
                <div className="flex-1 flex flex-col justify-between">
                  ...
                  <div className="mt-8 flex flex-col sm:flex-row sm:items-end justify-between gap-6">
                    ...
                    <div className="flex items-center gap-3">
                      {kit.camera.stock_quantity !== undefined && kit.camera.stock_quantity < selection.camera_count ? (
                        <div className="px-6 py-4 bg-red-50 dark:bg-red-900/10 border border-red-200 dark:border-red-900/50 rounded-2xl">
                           <span className="text-[10px] font-black text-red-600 dark:text-red-400 uppercase tracking-widest">
                             {kit.camera.stock_quantity <= 0 ? "Out of Stock" : `Only ${kit.camera.stock_quantity} left`}
                           </span>
                        </div>
                      ) : (
                        <>
