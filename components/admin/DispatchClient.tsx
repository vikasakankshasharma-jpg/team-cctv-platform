              ...
              displayJobs.map((job) => {
                ...
                return (
                  <TableRow key={job.id} className="border-border/50 hover:bg-muted/20">
                    <TableCell>
                      <div className="font-mono text-xs text-muted-foreground mb-1">{job.id?.slice(0, 10).toUpperCase()}</div>
                      {job.type === "survey" ? (
                        <Badge variant="outline" className="border-purple-500/50 text-purple-400 bg-purple-950/20 uppercase tracking-wider text-[10px] font-bold">
                          📅 Site Survey
                        </Badge>
                      ) : (
                        <Badge variant="outline" className="border-blue-500/50 text-blue-400 bg-blue-950/20 uppercase tracking-wider text-[10px] font-bold">
                          🛠️ {job.type?.replace("_", " ") || "Job"}
                        </Badge>
                      )}
                      {job.customer?.name && (
                    ...
                    <TableCell>
                      <div className="flex flex-col gap-1 text-sm">
                        <div className="flex items-center gap-1.5 font-medium">
                          ...
                          <span>{job.address?.pincode || (typeof job.address === "string" ? job.address : "Location")}</span>
                        </div>
                        {Boolean(job.scheduled_at) && (
                          <div className="text-[11px] text-purple-400 font-semibold">
                            ⏰ {String(job.scheduled_at)} {job.time_slot ? `(${job.time_slot})` : ""}
                          </div>
                        )}
                        {job.special_notes && (
