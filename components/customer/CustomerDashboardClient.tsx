...
export function CustomerDashboardClient({ user, quotes }: CustomerDashboardProps) {
  ...
  const [visibleInvoiced, setVisibleInvoiced] = useState(5);
  
  const invoicedQuotes = safeQuotes.filter(q => q && q.isPaid && (q.amountDue ?? 0) <= 0);
  const bookedQuotes = safeQuotes.filter(q => q && q.isPaid && (q.amountDue ?? 0) > 0);
  const unbookedQuotes = safeQuotes.filter(q => q && !q.isPaid);

  const [activeTab, setActiveTab] = useState<'active' | 'pending' | 'invoiced'>(
    bookedQuotes.length > 0 ? 'active' : unbookedQuotes.length > 0 ? 'pending' : invoicedQuotes.length > 0 ? 'invoiced' : 'pending'
  );
  ...
  const paidQuotesCount = bookedQuotes.length + invoicedQuotes.length;
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-2xl sm:rounded-3xl p-4 sm:p-8 border border-zinc-200 dark:border-zinc-800 shadow-sm flex flex-col sm:flex-row items-start sm:items-center justify-between gap-5 sm:gap-6">
          ...
          <div className="flex flex-row items-center gap-2 sm:gap-3 w-full sm:w-auto">
            ...
            <Link href="/customer/documents" className="flex-1 sm:flex-none flex items-center justify-center gap-1.5 text-center bg-indigo-50 dark:bg-indigo-950/50 border border-indigo-200 dark:border-indigo-900 text-indigo-700 dark:text-indigo-400 px-3 sm:px-4 py-2 sm:py-2.5 rounded-xl font-bold text-xs sm:text-sm transition-all whitespace-nowrap hover:bg-indigo-100 dark:hover:bg-indigo-900"><FileText className="w-4 h-4 shrink-0" />Documents</Link>
            <Link href="/customer/support" className="flex-1 sm:flex-none flex items-center justify-center gap-1.5 text-center bg-emerald-50 dark:bg-emerald-950/50 border border-emerald-200 dark:border-emerald-900 text-emerald-700 dark:text-emerald-400 px-3 sm:px-4 py-2 sm:py-2.5 rounded-xl font-bold text-xs sm:text-sm transition-all whitespace-nowrap hover:bg-emerald-100 dark:hover:bg-emerald-900"><Headphones className="w-4 h-4 shrink-0" />Support</Link>
            <Link
              href="/wizard"
