...
if (!content.includes('Wrench')) {
  content = content.replace(
    ...
    'Search, ChevronDown, ChevronRight, Edit2, Camera, Monitor, \n  Layers, Zap, Package, HardDrive, Cpu, ShieldCheck, Wrench, Shield, Tv, Box, Server,\n  ListFilter, Filter'
  );
}
...
// Replace Render Loop
const oldRender = `{["camera", "recorder", ...Object.keys(CATEGORIES).filter(k => !["camera", "recorder"].includes(k))].map((cat) => {
...
const newRender = `{/* MUST HAVE CATEGORIES */}
...
console.log("Patched ProductInventory.tsx correctly");
