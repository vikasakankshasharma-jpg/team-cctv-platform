...
for (const b of boundaries) {
  ...
  for (let i = b.start; i <= b.end; i++) {
    for (const [key, val] of Object.entries(dict)) {
      // match exactly the key in a line
      // format is like: `quote_h1: "some text",` or `'quote_h1': "some text",`
      const keyRegex = new RegExp(`^\\s*['"]?${key}['"]?:\\s*".*?"\\s*,?\\s*$`);
      if (keyRegex.test(lines[i])) {
        // replace just the value part
        lines[i] = lines[i].replace(/:\s*".*?"/, `: "${val}"`);
        // remove the key from dict so we know it was updated
        delete dict[key];
      }
    }
  }
  // Any keys that weren't found, add them to the end of the block
...
console.log("Successfully replaced translation strings using robust line-by-line method!");
