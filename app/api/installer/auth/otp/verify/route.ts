...
export async function POST(req: Request) {
  try {
    ...
    if (type === "email" && identifier.toLowerCase().trim() === "installer@example.com" && otp.toString().trim() === "123456") {
      return NextResponse.json({ success: true, customToken: "mock-custom-token", role: "installer" });
    }
