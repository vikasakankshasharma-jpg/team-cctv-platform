...
for (const locale of locales) {
  ...
  if (localeProp.isKind(SyntaxKind.PropertyAssignment)) {
    ...
    // Replace property values if they exist in realTranslations
    for (const [key, val] of Object.entries(realTranslations[locale])) {
      ...
      if (prop && prop.isKind(SyntaxKind.PropertyAssignment)) {
        prop.setInitializer(JSON.stringify(val));
      }
    }
  }
}
...
sourceFile.saveSync();
console.log('Real translations injected successfully.');
