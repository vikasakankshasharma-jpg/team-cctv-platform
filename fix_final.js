...
const extraMissingKeys = ['tab_tech', 'tab_cam', 'tab_rec', 'tab_sto', 'tab_pow', 'tab_acc', 'step_storage_desc', 'step_features_desc', 'step_general_addons_desc', 'wiz_sel_all', 'wiz_multi', 'No Storage Required', '3 Months'];

const missingKeys = [...Object.keys(dicts.hi), ...extraMissingKeys];

// Clean TranslationKey append:
let unionAppend = "";
...
// Ensure we remove the trailing semicolon on line 260
...
for (const lang of ['hi', 'gu', 'mr', 'ta']) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (inLang) {
      ...
      // If we've reached the end of the language block
      if (braces === 0) {
        ...
        // Append all the keys
        const newLines = Object.keys(dict).map(k => `      '${k}': "${dict[k]}",`);
...
console.log("Translations successfully updated with perfectly clean syntax.");
