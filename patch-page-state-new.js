...
// 3. Add floating action bar if missing. Safely replace </main>
if (!page.includes('selectedIds.size > 0 && (')) {
  ...
  // Use string replace to only replace the first occurrence of </main> from the bottom
  const parts = page.split('</main>');
  if (parts.length > 1) {
    const lastPart = parts.pop();
    page = parts.join('</main>') + fab + lastPart;
  }
}

fs.writeFileSync('app/(admin)/admin/products/page.tsx', page);
console.log("Restored state, handlers, and FAB safely!");
